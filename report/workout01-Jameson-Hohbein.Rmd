...
```{r, echo=FALSE}
knitr::include_graphics('../images/gsw-shot_chart.png')
url <- "https://www.gannett-cdn.com/presto/2018/10/29/USAT/2d9b02f9-c2ee-405e-bb80-1d2bc066388f-2018-10-28_Steph_Curry3.jpg"
```

#Players

##Stephen Curry
<center><img src="`r url`", width=200,height=200></center>
...
##Kevin Durant
```{r, echo=FALSE}
url <- "https://usatftw.files.wordpress.com/2018/06/usp_nba-_finals-golden_state_warriors_at_cleveland.jpg"
```
<center><img src="`r url`", width=400,height=400></center>
...
##Draymond Green
```{r, echo=FALSE}
url <- "https://thespun.com/wp-content/uploads/2018/05/GettyImages-857779692-775x465.jpg" 
```
<center><img src="`r url`", width=400,height=400></center>
...
##Klay Thompson
```{r, echo=FALSE}
url <- "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5c/Klay_Thompson_vs._Jared_Dudley_%28cropped%29.jpg/800px-Klay_Thompson_vs._Jared_Dudley_%28cropped%29.jpg" 
```
<center><img src="`r url`", width=200,height=200></center>
...
##Andre Iguodala
```{r, echo=FALSE}
url <- "http://c-3sux78kvnkay76x24osgmkyx2evkx78lux78smx78uavx2eius.g00.sportingnews.com/g00/3_c-3ccc.yvux78zotmtkcy.ius_/c-3SUXKVNKAY76x24nzzvx3ax2fx2fosgmky.vkx78lux78smx78uav.iusx2fjox2frohx78gx78ex2fyvux78zotm_tkcyx2fi5x2f5kx2fgtjx78k-omaujgrg-mkzze-778474-lzx78pvm_7umamnbc7rtld7ues3uwjcuskj.pvmx3fzx3d7109816175x26cx3d526x26wagrozex3d36x26o76i.sgx78qx3dosgmk_$/$/$/$/$/$/$" 
```
<center><img src="`r url`", width=400,height=400></center>
...
eshoot = data.frame(
  name=c("Stephen Curry","Kevin Durant","Draymond Green","Klay Thompson","Andre Iguodala"),
  total=0,
  made=0,
  perc_made=0
)
for (n in eshoot$name){
  eshoot[eshoot$name==n,]$total = length(filter(shotsdata, shotsdata$name==n)$shot_made_flag)
  eshoot[eshoot$name==n,]$made = length(filter(shotsdata, shotsdata$name==n, shotsdata$shot_made_flag=="shot_yes")$shot_made_flag)
  eshoot[eshoot$name==n,]$perc_made = (eshoot[eshoot$name==n,]$made / eshoot[eshoot$name==n,]$total)*100
}

eshoottwo = data.frame(
  name=c("Stephen Curry","Kevin Durant","Draymond Green","Klay Thompson","Andre Iguodala"),
  total=0,
  made=0,
  perc_made=0
)
for (n in eshoottwo$name){
  eshoottwo[eshoottwo$name==n,]$total = length(filter(shotsdata, shotsdata$name==n,shotsdata$shot_type=="2PT Field Goal")$shot_made_flag)
  eshoottwo[eshoottwo$name==n,]$made = length(filter(shotsdata, shotsdata$name==n,shotsdata$shot_type=="2PT Field Goal", shotsdata$shot_made_flag=="shot_yes")$shot_made_flag)
  eshoottwo[eshoottwo$name==n,]$perc_made = (eshoottwo[eshoottwo$name==n,]$made / eshoottwo[eshoottwo$name==n,]$total)*100
}

eshootthree = data.frame(
  name=c("Stephen Curry","Kevin Durant","Draymond Green","Klay Thompson","Andre Iguodala"),
  total=0,
  made=0,
  perc_made=0
)
for (n in eshootthree$name){
  eshootthree[eshootthree$name==n,]$total = length(filter(shotsdata, shotsdata$name==n,shotsdata$shot_type=="3PT Field Goal")$shot_made_flag)
  
  eshootthree[eshootthree$name==n,]$made = length(filter(shotsdata, shotsdata$name==n,shotsdata$shot_type=="3PT Field Goal", shotsdata$shot_made_flag=="shot_yes")$shot_made_flag)
  
  eshootthree[eshootthree$name==n,]$perc_made = (eshootthree[eshootthree$name==n,]$made / eshootthree[eshootthree$name==n,]$total)*100
}
...
kable(arrange(eshoottwo,desc(eshoottwo$perc_made)),caption="Total Two Shots")
```
...
```{r out.width='80%', echo= FALSE, fig.align='center'}
kable(arrange(eshootthree,desc(eshootthree$perc_made)),caption="Total Three Shots")
```
...
```{r out.width='80%', echo= FALSE, fig.align='center'}
kable(arrange(eshoot,desc(eshoot$perc_made)),caption="Total Shots")
```
